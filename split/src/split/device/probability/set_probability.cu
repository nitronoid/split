                    ...
                    const int max_length)
{
  ...
  thrust::exclusive_scan(
    segment_lengths.begin(), segment_lengths.end() - 1, segment_lengths.begin());
  // Reset the keys
  ...
  const auto scatter_ends = 
    thrust::make_permutation_iterator(key_begin, thrust::make_transform_iterator(
  ...
  auto new_end = thrust::remove_if(key_value_begin, 
                                   key_value_end,
  ...
  cusp::array1d<real, cusp::device_memory>::view do_probability)
{
  ...
  const int nsets = 
    *thrust::max_element(dio_set_labels.begin(), dio_set_labels.end());
  ...
  const auto set_albedo_begin = thrust::make_permutation_iterator(
                   albedo_begin, dio_set_ids.begin());
  ...
  auto probabilities_out = thrust::make_permutation_iterator(
      thrust::make_transform_output_iterator(
        do_probability.begin(), detail::unary_minus<real>(1.f)), 
